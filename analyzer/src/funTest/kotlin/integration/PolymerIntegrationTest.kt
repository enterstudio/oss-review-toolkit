...
class PolymerIntegrationTest : AbstractIntegrationSpec() {
    ...
    override val expectedDefinitionFiles by lazy {
        mapOf(
                Bower to listOf(File(downloadDir, "bower.json")),
                NPM to listOf(File(downloadDir, "package.json"))
        )
    }

    override val definitionFilesForTest by lazy {
        mapOf(Bower as PackageManagerFactory<PackageManager> to listOf(File(downloadDir, "bower.json")))
    }
}
